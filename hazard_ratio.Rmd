...
First we make a survival function for treatment A.   The survival function gives us, for any time $t$, the proportion of people still alive at time $t$.  This survival function is an idealized mathematical calculation, but let's pretend the values from the function represent actual measured survival at any particular time $t$.
...
The detail of the actual equation in `survival_a` is not much relevant here - it's just a way to generate a typical-looking survival curve.
...
This tells us that around 19% of people survived 5 years or more on treatment A.


In fact, the way we've written the function, we can also send it a sequence of values for $t$, and it will calculate survival for all the values:
...
Next we make an array to store *all* the times for which we imagine having measured survival, in our idealized example.
...
We plot survival for A, by asking for survival for all our example times in the `times` array above:
...
This tells us that those having treatment A have 0.82 (82%) of the risk of death
suffered with treatment B.  Put another way, A has an 18% relative reduction in death
...
the death value after a short time interval, divided by the length of time interval
([slope](https://en.wikipedia.org/wiki/Slope) is change in y-axis value divided
by change in x-axis value, or *rise over run*).

Here is the death *rate* for A at 5 years, with a time interval of 0.001 years:

```{python}
death_A_5 = 1 - survival_a(5)  # Death at 5 years.
death_A_5p001 = 1 - survival_a(5.001)  # Death at 5.001 years.
# Slope is change in y (above) divided by change in x (0.001).
death_rate_A_5 = (death_A_5p001 - death_A_5) / 0.001
...
death_A = 1 - survival_a(times)
# Death rate for these times, each plus 0.001.
death_Ap001 = 1 - survival_a(times + 0.001)
# Rates (slopes) for all times.
death_rate_A = (death_Ap001 - death_A) / 0.001
...
```{python}
death_B = 1 - survival_b(times)
death_Bp001 = 1 - survival_b(times + 0.001)
death_rate_B = (death_Bp001 - death_B) / 0.001
...
In our example, the hazard rates are the same at all times, for both treatments.  This is a mathematical artefact of our use of simple exponential functions to create the initial survival curves.  In general, with more complicated survival curves, the hazard rates can vary over time.
...
a survival value.  In our example above, $S_A$ is the mathematical notation for our Python function `survival_a`. Thus $S_A(t)$ is the survival proportion
